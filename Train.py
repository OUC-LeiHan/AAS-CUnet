...
from utils.ERA5_Dataset import ERA5_Dataset
from utils.plot_fun import *
from utils.My_loss import  *
from ERA5_net.ERA5_Unet import UNet
from eval import eval_net_EC
from natsort import natsorted
import time
...
def train_net(net,epochs=5,batch_size=1,
    ...
    train_Data = ERA5_Dataset(path_For_Train, "Train", [48,48], min_t2m=0, max_t2m=1)
    test_Data = ERA5_Dataset(path_For_Test, "Test", [48,48], min_t2m=0, max_t2m=1)
    ...
    for epoch in range(epochs):
        ...
        test_loss, test_mae = eval_net_EC(net=net, dataset=test_Data_Loader, save_result_dir=dir_result, iter_index=epoch,
                                                  Scale_Grid=Grid_NUM, plot_Sample=plot_Sample, train_loss=epoch_loss/epoch_index, min_t2m=0, max_t2m=1)
        if save_cp:
...
#Using command line parameters, you can modify training parameters
def get_args():
    parser = OptionParser()
    ...
    parser.add_option("-i", "--test_interval", dest="test_interval", default=4000,
                      help="test interval")
