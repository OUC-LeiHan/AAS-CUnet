...
代码说明
########################################################################################
ERA5_net：包括网络的核心代码，EC_model_parts.py描述网络子模块，ERA5_Unet.py描述网络总体搭建过程。
checkpoint: 保存网络训练的中间模型，后续进行模型测试。
log：存储网络训练日志以及训练过程中的误差曲线等。
utils：包括数据集制作、读取、日志生成、曲线绘制等功能性代码。
          其中ERA5_Dataset.py描述数据集制作以及数据读取。
                 My_loss.py描述损失函数构造
                 Tools.py 描述评分指标计算方式
                 log_txt.py 描述日志生成
                 plot_fun.py描述误差曲线绘制
...
Description
##############################################################################################
ERA5_net：Including the core code of the network, EC_ model_ parts.py description network sub module, ERA5_ Unet.py describes the 
...
utils：Including functional codes such as  ERA5_Dataset.py、My_loss.py、Tools.py、log_txt.py、 plot_fun.py.

          ERA5_Dataset.py：Describe the process of making and reading dataset.
...
eval.py: Verify the model effect.
###########################################################################################